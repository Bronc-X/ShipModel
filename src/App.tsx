...
const statusMeta: Record<CompetitionStatus, { label: string; tone: string; action: string }> = {
  open: { label: "报名中", tone: "blue", action: "进入报名系统" },
  running: { label: "比赛中", tone: "red", action: "查看实时成绩" },
  upcoming: { label: "即将开始", tone: "neutral", action: "关注赛事动态" },
  completed: { label: "已完赛", tone: "muted", action: "查看最终成绩" }
};
...
const competitions: Competition[] = [
  {
    ...
    summary: "面向全国航空模型运动员与青少年队伍的公开赛，设置特技、滑翔与综合技能项目。",
  ...
  },
  {
    ...
    organizer: "上海市航空运动协会",
    summary: "面向多旋翼竞速飞手的分站公开赛，提供实时成绩与赛道公告。",
  ...
  },
  {
    ...
    summary: "聚焦青少年科技体育教育，覆盖自由飞、遥控飞行和航天模型项目。",
  ...
  },
  {
    ...
    summary: "以航天模型设计、发射与回收为核心的创新类赛事，已完成成绩公示。",
...
const articles: Article[] = [
  {
    ...
    source: "Lusie 赛事服务中心",
    summary: "南京站竞赛规程、报名截止时间、器材检录要求和参赛队报到材料已发布。",
  ...
  },
  {
    ...
    source: "上海站竞赛委员会",
    summary: "专业组与校园组预赛轮次成绩已同步，最终成绩以裁判长签字版本为准。",
  ...
  },
  {
    ...
    source: "Lusie 技术支持",
    summary: "说明机构账号登录、批量导入队员、提交项目材料和审核状态查询路径。",
    attachment: "机构组织报名操作手册.pdf",
    sections: [
      { title: "适用对象", body: "学校、俱乐部、培训机构和协会分支机构均可按本说明组织报名。" },
      { title: "老系统交接", body: "机构登录仍由原业务系统承接，新前端只提供清楚入口和操作说明。" }
    ]
  },
  {
    ...
    source: "Model 学院",
    summary: "按年龄、器材、场地和训练周期拆解适合新手的项目选择。",
  ...
  },
  {
    ...
    source: "赛事媒体组",
    summary: "回顾分站赛中的赛道调度、检录流程和优秀飞手表现。",
...
const faqs: FAQItem[] = [
  { id: "account-1", topic: "account", question: "忘记登录密码怎么办？", answer: "请在老业务系统登录页使用找回密码功能；如手机号已停用，请联系账号与技术支持并提供身份证明。" },
  { id: "registration-1", topic: "registration", question: "个人和机构报名有什么区别？", answer: "个人报名适合独立参赛者；学校、俱乐部和协会队伍建议使用机构账号统一导入队员并提交材料。" },
  { id: "upload-1", topic: "upload", question: "作品或器材资料上传后可以修改吗？", answer: "报名截止前可以在原报名系统内修改；进入审核后需要联系赛务人员退回后再重新提交。" },
  { id: "review-1", topic: "review", question: "审核状态一直没有变化怎么办？", answer: "先确认材料是否完整。超过 3 个工作日仍未更新，可联系报名与赛务咨询渠道。" },
  { id: "score-1", topic: "score", question: "成绩公示和最终成绩有什么区别？", answer: "成绩公示用于申诉期确认，最终成绩以裁判长签字并发布的成绩册为准。" },
  { id: "certificate-1", topic: "certificate", question: "证书在哪里下载？", answer: "证书下载由老业务系统承接。请从赛事详情页的证书入口进入，并使用参赛报名账号登录。" },
  { id: "institution-1", topic: "institution", question: "学校老师如何批量组织报名？", answer: "请先申请或使用机构账号登录，下载机构报名模板，批量导入队员后提交项目材料。" }
];

const serviceChannels = [
  { title: "报名与赛务咨询", icon: <ClipboardCheck size={22} />, email: "reg@lusie.org", phone: "010-1234-5678 转 1", note: "报名流程、资格审核、赛程安排" },
  { title: "账号与技术支持", icon: <LifeBuoy size={22} />, email: "tech@lusie.org", phone: "010-1234-5678 转 2", note: "登录异常、资料上传、成绩或证书入口" },
  { title: "机构合作与招商", icon: <Building2 size={22} />, email: "bd@lusie.org", phone: "010-1234-5678 转 3", note: "承办单位、学校机构、器材服务商合作" },
  { title: "媒体与公关咨询", icon: <Newspaper size={22} />, email: "pr@lusie.org", phone: "010-1234-5678 转 4", note: "采访申请、影像授权、官方发布" }
];
    ...
    <header className="site-header">
      <div className="header-inner">
        ...
        <div className="header-actions">
          ...
          <AppLink className="button primary small" to={legacyUrls.participantRegister}>注册</AppLink>
  ...
  return (
    <>
      ...
      <section className="page-section page-container">
        <SectionHeader eyebrow="Competition Center" title="当前赛事" action={<AppLink to="/competitions">查看全部 <ArrowRight size={16} /></AppLink>} />
      ...
      <section className="page-section muted-band">
        <div className="page-container split-layout">
          <div>
            <SectionHeader eyebrow="Official Notices" title="通知、成绩与资料分离" />
      ...
      <section className="page-section page-container">
        <div className="service-grid">
          <FeaturePanel
            icon={<Trophy size={24} />}
            title="成绩与证书"
            copy="成绩公示、最终成绩和证书入口分开呈现，减少用户在新闻列表里找资料的成本。"
          ...
          <FeaturePanel
            ...
            title="Model 学院轻量版"
            copy="先提供项目选择、器材建议、评审标准和优秀作品，不把用户带到空页面。"
            to="/academy"
            action="进入学院"
          />
          <FeaturePanel
            ...
            title="机构组织报名"
            copy="保留老业务系统入口，并明确说明将跳转到机构后台完成批量报名。"
...
function QuickAccess() {
  const quickLinks = [
    { title: "赛事报名", icon: <UserPlus size={24} />, to: "/competitions", note: "查找可报名赛事" },
    { title: "规则下载", icon: <Download size={24} />, to: "/downloads", note: "PDF / 表格直达" },
    { title: "成绩查询", icon: <Trophy size={24} />, to: "/news?category=score", note: "成绩公示与最终成绩" },
    { title: "证书查询", icon: <Award size={24} />, to: legacyUrls.certificate, note: "跳转老系统" },
    { title: "联系客服", icon: <LifeBuoy size={24} />, to: "/contact", note: "按问题类型联系" }
  ];
  ...
  return (
    <div className="page-container page-stack">
      <PageTitle
        ...
        title="赛事中心"
        copy="浏览、筛选并进入全国各级航空模型赛事。报名提交、证书下载等核心业务继续由老系统承接。"
      />
      <section className="filter-panel">
        ...
        <div className="filter-controls">
          <label className="search-box">
            <Search size={18} />
            <input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索赛事名称、地点或组别" />
          </label>
      ...
      {filtered.length ? (
        ...
        <EmptyState title="没有找到符合条件的赛事" copy="请调整状态、类别或搜索关键词；也可以查看报名指南了解下一步。" action={<AppLink className="button primary" to="/guide/registration">查看报名指南</AppLink>} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="page-container page-stack">
      <div className="login-banner">
        <Info size={21} />
        <p>登录后可报名、查看个人成绩或下载证书。报名提交和证书下载会跳转到原业务系统。</p>
      ...
      <section className="detail-hero">
        ...
        <div className="detail-action-card">
          ...
          <div className="button-column">
            <AppLink className="button danger" to={competition.registrationUrl}>进入报名系统 <ExternalLink size={17} /></AppLink>
            <AppLink className="button ghost" to={competition.rulesUrl}>下载规则文件 <Download size={17} /></AppLink>
      ...
      <div className="detail-layout">
        <div className="detail-main">
          <section className="content-card">
            <SectionHeader eyebrow="Overview" title="赛事简介" />
            <p>{competition.summary} 页面将规则、报名、成绩、证书和老系统交接分成独立模块，移动端也能按任务顺序阅读。</p>
          </section>
          <section className="content-card">
            <SectionHeader eyebrow="Documents" title="规则与附件" />
          ...
          <section className="content-card">
            <SectionHeader eyebrow="Schedule" title="日程安排" />
          ...
          <section className="content-card">
            <SectionHeader eyebrow="Results" title="成绩 / 奖项预览" action={<AppLink to={competition.resultUrl}>查看完整成绩 <ArrowRight size={16} /></AppLink>} />
            {competition.results.length ? <ResultsTable rows={competition.results} /> : <EmptyState compact title="成绩尚未发布" copy="比赛开始后，成绩公示会在这里显示；最终成绩以官方成绩册为准。" />}
          </section>
        </div>
        <aside className="detail-sidebar">
          ...
          <section className="content-card support-card">
            <LifeBuoy size={24} />
            <h3>赛事支持</h3>
            <p>不确定报名、规则或成绩入口？按问题类型联系对应支持渠道。</p>
            <AppLink className="button primary" to="/contact">联系支持</AppLink>
          </section>
        </aside>
      </div>
    </div>
...
function NewsList({ search }: { search: string }) {
  ...
  return (
    <div className="page-container page-stack">
      <PageTitle eyebrow="News Center" title="新闻公告中心" copy="官方通知、赛事资讯、成绩公示、资料下载和媒体回顾分开浏览。" />
      <section className="news-layout">
        <div>
          <div className="filter-panel news-filter">
            ...
            <label className="search-box">
              <Search size={18} />
              <input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索标题、摘要或附件" />
            </label>
          </div>
...
function DownloadCenter() {
  ...
  return (
    <div className="page-container page-stack">
      <PageTitle eyebrow="Download Center" title="下载中心" copy="资料直接以表格和卡片方式展示，不再藏在新闻正文末尾。" />
      <section className="filter-panel">
        <label className="search-box">
          <Search size={18} />
          <input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索资料名称或所属赛事" />
        </label>
        <label className="select-box">
          <FileText size={18} />
      ...
      <section className="download-table" aria-label="下载资料列表">
        ...
        {filtered.map((resource) => (
          <div className="download-row" key={resource.id}>
            ...
            <AppLink className="button danger small" to={resource.url}>下载 <Download size={16} /></AppLink>
          </div>
        ))}
      </section>
    </div>
    ...
    <div className="page-container page-stack">
      <section className="guide-hero">
        <div>
          ...
          <h1>6 步完成赛事报名</h1>
          <p>先看清流程，再进入原业务系统提交报名。这里保留所有关键入口和材料准备说明。</p>
        </div>
        <div className="button-row">
          <AppLink className="button danger" to={legacyUrls.participantRegister}>直接报名 <ExternalLink size={17} /></AppLink>
          <AppLink className="button ghost" to="/downloads">下载操作手册 <Download size={17} /></AppLink>
        </div>
      </section>
      ...
      <section className="recommend-layout">
        <div>
          <SectionHeader eyebrow="Project Finder" title="我适合先参加什么项目？" />
        ...
        <FeaturePanel icon={<HelpCircle size={24} />} title="仍然不确定？" copy="FAQ 覆盖账号、报名、上传、审核、成绩、证书和机构组织报名。" to="/faq" action="查看 FAQ" />
      </section>
    </div>
  );
}
...
function FAQPage() {
  ...
  return (
    <div className="page-container page-stack">
      <PageTitle eyebrow="FAQ" title="常见问题" copy="按账号、报名、上传、审核、成绩、证书和机构报名拆分，减少重复咨询。" />
      <section className="filter-panel">
        <div className="tabs scroll-tabs">
        ...
        <label className="search-box">
          <Search size={18} />
          <input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索问题关键词" />
        </label>
      </section>
    ...
    <div className="page-container page-stack">
      <section className="academy-hero">
        <div>
          ...
          <h1>先做轻量可用的学习入口</h1>
          <p>学院不再落到“页面升级中”。第一版提供项目选择、器材建议、作品案例和评审说明。</p>
          <AppLink className="button primary" to="/guide/registration">从报名流程开始</AppLink>
        </div>
        <img alt="航空模型学习与制作" src={images.classroom} />
      </section>
      <section className="academy-grid">
        <AcademyCard icon={<Plane size={24} />} title="我应该先参加哪个比赛？" copy="按年龄、经验、训练周期和器材预算推荐项目。" />
        <AcademyCard icon={<BookOpen size={24} />} title="需要什么器材？" copy="列出新手练习机、遥控器、电池、安全工具和检录材料。" />
        <AcademyCard icon={<CheckCircle2 size={24} />} title="作品如何评审？" copy="解释安全性、完成度、飞行表现和创新设计的评分口径。" />
        <AcademyCard icon={<PlayCircle size={24} />} title="好作品长什么样？" copy="沉淀近年常青作品案例和赛事回放，不再用陈旧内容充数。" />
      </section>
      <section className="content-card">
        <SectionHeader eyebrow="Coming Later" title="后续上线，但不占主视觉" />
        <div className="chip-cloud">
          <span>在线课程</span>
          <span>附近机构</span>
          <span>器材选择工具</span>
...
function AboutPage() {
  return (
    <div className="page-container page-stack">
      <PageTitle eyebrow="About Lusie" title="关于我们" copy="官方、克制、可验证地说明平台职责、服务对象和旧系统交接边界。" />
      <section className="about-grid">
        <div className="content-card about-main">
          <h2>平台简介</h2>
          <p>Lusie 航空模型竞赛服务平台面向赛事主办方、承办方、参赛选手、学校机构和裁判团队，提供公开展示、赛事信息分发、资料下载、成绩公告和服务支持入口。</p>
          <p>第一版公开前端不重建登录、报名提交、证书生成和机构后台，而是用清楚的交接入口连接现有业务系统。</p>
        </div>
        <FeaturePanel icon={<ShieldCheck size={24} />} title="平台职责" copy="统一展示赛事信息、规程资料、公告、成绩和联系路径。" to="/organization" action="查看组织说明" />
        <FeaturePanel icon={<Users size={24} />} title="服务对象" copy="参赛选手、学校老师、俱乐部、裁判员、承办方和媒体合作方。" to="/contact" action="联系渠道" />
      </section>
    </div>
  );
}
...
function ContactPage() {
  return (
    <div className="page-container page-stack">
      <PageTitle eyebrow="Contact" title="联系我们" copy="按问题类型提供联系方式、服务时间和扫码用途说明。" />
      ...
      <section className="contact-bottom-grid">
        ...
        <div className="content-card qr-panel">
          ...
          <div>
            <h3>官方媒体矩阵</h3>
            <p>扫码获取赛事公告、报名提醒和手机端查分入口。二维码位由运营素材接入。</p>
          </div>
    ...
    <div className="page-container policy-layout">
      ...
      <article className="policy-article">
        <header>
          ...
          <h1>{policy.title}</h1>
          <p>最近更新：{policy.updatedAt} · 适用范围：Lusie 公开展示层与相关赛事服务入口。</p>
        </header>
        {policy.sections.map((section, index) => (
...
const policyContent: Record<PolicyKind, { label: string; title: string; updatedAt: string; sections: Array<{ title: string; body: string }> }> = {
  privacy: {
    ...
    sections: [
      { title: "引言", body: "我们仅为赛事报名、成绩公告、证书查询和平台安全所需处理必要信息。" },
  ...
  terms: {
    ...
    updatedAt: "2026-05-01",
    sections: [
      { title: "服务范围", body: "公开展示层提供赛事信息、资料下载、公告和支持入口，业务提交由原系统承接。" },
  ...
  organization: {
    ...
    updatedAt: "2026-05-01",
    sections: [
      { title: "平台角色", body: "Lusie 公开展示层承担赛事信息展示、资料分发和服务支持入口职责。" },
      { title: "赛事组织", body: "具体赛事由主办、承办和竞赛委员会负责执行，页面会展示对应单位信息。" },
      { title: "业务系统", body: "登录、报名提交、证书下载和机构后台继续由原业务系统承接。" }
    ]
  },
  brand: {
    label: "Brand",
...
function NotFoundPage() {
  return (
    <div className="page-container page-stack">
      <EmptyState
        title="页面未找到"
        copy="这个地址暂未映射到新版公开展示层。你可以回到首页或进入赛事中心。"
  ...
  return (
    <div className="page-container page-stack">
      <section className="legacy-handoff">
        ...
        <h1>即将进入原{kind}系统</h1>
        <p>新版公开展示层负责赛事信息、资料下载、公告和支持入口。这个动作仍由原业务系统承接，生产环境会按该地址交接。</p>
        <code>{fullPath}</code>
        <div className="button-row">
    ...
    <footer className="site-footer">
      <div className="page-container footer-grid">
        <div>
          ...
          <p>航空模型竞赛服务公开展示层。报名、登录、证书等流程由现有业务系统承接。</p>
        </div>
...
function GuidePreview() {
  return (
    <div className="guide-preview content-card">
      ...
      <AppLink className="button primary" to="/guide/registration">查看完整指南</AppLink>
    </div>
  );
}
